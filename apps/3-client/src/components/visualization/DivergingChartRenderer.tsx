...
export default function GanttChartRenderer({
    ...
    const renderTaskLabel = useCallback(({ x, y, width, height, index }: any) => {
        ...
        if (!task) return <g></g>; // Trả về element rỗng thay vì null
                    ...
                    status && (
                        <div key={index} className="flex items-center">
                            <div
                                ...
                                style={{ backgroundColor: statusColors[status as keyof typeof statusColors] || COLORS[index % COLORS.length] }}
                            />
                            <span>{status}</span>
                        </div>
                    )
    ...
    const ganttChartData = useMemo(() => {
        if (groupTasks) {
            ...
            const grouped: Record<string, GanttTask[]> = chartData.reduce((acc, task) => {
                if (!acc[task.group]) {
                    acc[task.group] = [];
                }
